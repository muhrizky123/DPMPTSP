...
with st.spinner('Updating Report .... ') : 
    ...
    m1.write('')
    m2.metric(label = "Total Izin yang diajukan", value = total_izin.iloc[0])
    ...
    c3.metric(label = "Selesai diproses", value = selesai_diproses.iloc[0], delta=f"{round(selesai_perc.iloc[0], 1)}%")
    c4.metric(label = "Masih diproses", value = masih_diproses.iloc[0], delta=f"{round(masih_perc.iloc[0], 1)}%")
    c5.metric(label = "Ditolak & dibatalkan", value = ditolak_dibatalkan.iloc[0], delta=f"{round(ditolak_perc.iloc[0], 1)}%")
